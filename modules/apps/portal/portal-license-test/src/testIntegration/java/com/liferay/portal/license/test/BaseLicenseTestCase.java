...
public abstract class BaseLicenseTestCase {
	...
	public File deployFreeTierLicense(long validityPeriod) throws Exception {
		...
		return _buildBinaryFile(
			PRODUCT_ID_PORTAL, _FREE_TIER_ACCOUNT_NAME, _FREE_TIER_PRODUCT_NAME,
	...
	public void resetLicenseData() throws Exception {
		...
		LicenseManagerUtil.checkLicense(PRODUCT_ID_CMP);
	}
	...
	protected static final String PRODUCT_ID_CMP =
		"f37efde7-11b1-6ad7-5c60-07bec3334db1";

	protected static final String PRODUCT_ID_PORTAL = "Portal";
	...
	private static class ReflectionsHolder {
		...
		static {
			...
			if (_licenseManagerHelperClass != null) {
				String lifecycleActionField = PropsUtil.get(
					"license.test.lifecycle.action.field");
				String validateMethod = PropsUtil.get(
					"license.test.validate.method");
				String versionMethod = PropsUtil.get(
					"license.test.version.method");

				if (Validator.isNull(lifecycleActionField) ||
					Validator.isNull(validateMethod) ||
					Validator.isNull(versionMethod)) {

					throw new IllegalArgumentException(
						StringBundler.concat(
							"Please set portal properties \"license.test.",
							"lifecycle.action.field\", \"license.test.",
							"method\" and \"license.test.version.method\""));
				}

				try {
					_lifecycleActionField = _findField(
						classLoader, lifecycleActionField);
					_validateMethod = _findMethod(classLoader, validateMethod);
					_versionMethod = _findMethod(classLoader, versionMethod);
